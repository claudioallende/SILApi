...
namespace ResourceServer.Models
{
    public class ServicioMediadorSTOPandSIL
    {
        ...
        public IList<ICuposAgrupados> CuposAgrupadosPendientesDeAgregarEnSIL()
        {
            ...
            try
            {
                ...
                foreach (var consignacion in compPuertGranoVendCentro)
                {
                    ...
                    Puerto puerto = this.ObtenerPuertoPorIdTerminal(consignacion.Idterminal, session);//verificar si tambien debe filtrar por CUITDESTINO
                    conjuntoDeCupos.PuertoCta = puerto != null ? puerto.Cuenta : 0;
                    conjuntoDeCupos.Puerto = puerto != null ? puerto.Nombre : "";
        ...
        public List<NuevoCupoViewModelDTO> CuposPendientesDeAgregarEnSIL(NuevoCupoViewModelDTO datosDeFiltro = null)
        {
            ...
            try
            {
                ...
                foreach (var consignacion in consignaciones)
                {
                    ...
                    //Puerto puerto = this.ObtenerPuerto(consignacion.Cuitdestino, session);
                    Puerto puerto = this.ObtenerPuertoPorIdTerminal(consignacion.Idterminal, session); //ver si ademas debo consultar por cuitdestino
                    conjuntoDeCupos.Puerto = (puerto != null) ? puerto.Cuenta.ToString() : "";
                    conjuntoDeCupos.PuertoNombre = (puerto != null) ? puerto.Nombre.ToString() : "";
        ...
        /// <param name="nroCuit"></param>
        /// <returns></returns>
        private Puerto ObtenerPuertoPorIdTerminal(long idTerminal, ISession session)
        {
            if (idTerminal != 0)
            {
                PuertoStore puertoStore = new PuertoStore();
                Puerto puerto = puertoStore.FindByIdTerminal(idTerminal, session);
                return puerto;
            }
            return null;
        }
        ...
        public bool AgregarCupo(string alfa)
        {
            if (alfa != null && alfa.Length>0)
            {
                try
                {
                    ...
                    Puerto puerto = this.ObtenerPuertoPorIdTerminal(CupoInSTOP.Idterminal, session);//ver siademas debo filtrar por Cuitdestino
                    conjuntoDeCupos.Puerto = (puerto != null) ? puerto.Cuenta.ToString() : "";
