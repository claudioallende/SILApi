...
namespace ResourceServer.Models.DataAccess
{
  public class CuposStore : ICuposStore
  {
    ...
    public IList<Cupos> FindByCompradorAndVendedorAndGranoAndPuertoAndConsignacionAndInformadoAndDistribuidoBetweenDates(long CuentaComprador, long CuentaVendedor, int CodigoGrano, long CuentaPuerto, Consignacion Consignacion, DateTime FechaInicial, DateTime FechaFin, ISession Session)
    {
      ...
      // Utilizado por el envio de email a vendedor, por lo que debe ignorar el contacto comercial de la consignacion
      Result = Consignacion.FiltroConsignacion(Result, true);
    ...
    public IList<Cupos> FindByContactoComercialCompradorAndVendedorAndGranoAndPuertoAndConsignacionAndInformadoAndDistribuidoBetweenDates(string ContactoComercial, long CuentaComprador, long CuentaVendedor, int CodigoGrano, long CuentaPuerto, Consignacion Consignacion, DateTime FechaInicial, DateTime FechaFin, ISession Session)
    {
      var Result = Session.Query<Cupos>()
          .Where(x => x.ContactoComercial == ContactoComercial && x.Vendcta == CuentaVendedor && x.Compcta == CuentaComprador && x.Grano == CodigoGrano && x.Puerto == CuentaPuerto && x.Pdf == 0 && x.Status == 4 && x.Vendcta != x.Vendcyo);
